<script>
	import { addTodo, getTodos } from '../firestore';
	...
	const addTask = () => {
		if (store !== null) {
			...
			if (task != '') {
				...
				task = '';
				getList();
			} // stops user from adding an empty task
			else {
				error = 'Task is empty';
			}
		}
...
<div class="container">
	...
	<button class="uppercase px-1 py-1 text-xs text-gray-500 hover:text-black" on:click={addTask}
		>add task</button
	>
